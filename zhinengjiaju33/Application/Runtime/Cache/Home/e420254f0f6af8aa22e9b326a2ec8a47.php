		...
		<div class="Situation">
			<div class="leftbox">
				<div class="sub-nav">
					...
					<ul>
						<?php if(is_array($navsecond)): $k = 0; $__LIST__ = $navsecond;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$navse): $mod = ($k % 2 );++$k; if($k == 1): ?><li class="on">
									<a href="<?php echo ($navse["nav_url"]); ?>/nav_id/<?php echo ($navse["nav_id"]); ?>" target="__black"><?php echo ($navse["nav_name"]); ?></a><b>&gt;</b>
								</li>
								<?php else: ?>
								<li>
									<a href="<?php echo ($navse["nav_url"]); ?>/nav_id/<?php echo ($navse["nav_id"]); ?>" target="__black"><?php echo ($navse["nav_name"]); ?></a><b>&gt;</b>
			...
			<div class="Situation-right">
				<?php if(is_array($navsecond)): $k = 0; $__LIST__ = $navsecond;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$second): $mod = ($k % 2 );++$k; if($k == 1): ?><div>
							<div class="SituationRight-top">
								<span>您当前位置：<a href="/index.php/Home/Index/index">首页</a> > <?php echo ($navben["nav_name"]); ?> > <?php echo ($second["nav_name"]); ?></span>
								<h1><?php echo ($second["nav_name"]); ?></h1>
							</div>
							<div class="SituationRight-bot">
								<?php if(is_array($all_primary_work[$k-1])): $i = 0; $__LIST__ = $all_primary_work[$k-1];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$primary_article): $mod = ($i % 2 );++$i;?><p><?php echo ($primary_article["pa_content"]); ?></p><?php endforeach; endif; else: echo "" ;endif; ?>
