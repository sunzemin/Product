...
<script type="text/javascript">
	$(function(){
		...
		<div class="CentMenu">
			<?php if(is_array($product_classify_res)): $k = 0; $__LIST__ = $product_classify_res;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$product_classify): $mod = ($k % 2 );++$k; if($k == 1): ?><a href="javascript:;" class="MenuBackColo"><?php echo ($product_classify["pc_name"]); ?>
						<input type="hidden" class="pc_id" value="<?php echo ($product_classify["pc_id"]); ?>">
					</a>
					<?php else: ?>
					<a href="javascript:;">
						<?php echo ($product_classify["pc_name"]); ?>
						<input type="hidden" class="pc_id" value="<?php echo ($product_classify["pc_id"]); ?>">
		...
		<div class="CustomerCase" style="background: url(/Public/Home/img/img39.png) no-repeat;background-size: 100% 100%;">
			<div class="CustomerCaseCent">
				<?php if(is_array($caseData)): $i = 0; $__LIST__ = $caseData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$case): $mod = ($i % 2 );++$i; if($case["url"] == ''): ?><a href="/index.php/Home/Index/xinwenxiangqing/id/<?php echo ($case["id"]); ?>/nav_id/<?php echo ($nav_id); ?>" target="_blank">
		...
		<input type="hidden" value="<?php echo ($product_classify_res[0]["pc_id"]); ?>" class="inpc_id"/>
		<!--公共底部-->
		<footer>
	<div class="footerEnd">
