...
class PublicController extends Controller{
    ...
    public function selquanwu($nav_id,$p=1){
        ...
        $allarr[]=$lcarr3;
        //dump($allarr);exit;
        $this->ajaxReturn(array('alldatall'=>$allarr,'allcount'=>$allcount,'allpage'=>$allpage));
    ...
    public function productdata($nav_id,$p=1){
    	$product_classifyM=D('product_classify');
	    $product_classify_res=$product_classifyM->order('pc_sort desc')->select();
    	$navM=D('nav');
    	...
    	if($navben['nav_fid']==0){
    		$wherepcid['pc_id']=$product_classify_res[0]['pc_id'];
    	}else{
    		$wherepcnav['nav_id']=$nav_id;
    		$product_classify11=$product_classifyM->where($wherepcnav)->find();
    		$wherepcid['pc_id']=$product_classify11['pc_id'];
    	}
    	//分类信息
    	$product_classify=$product_classifyM->where($wherepcid)->find();
    	//dump($product_classify);
    	$productM=D('product');
        $whereAC['pc_id']=$product_classify['pc_id'];
        $product_res=$productM->where($whereAC)->order('psort desc')->page($p,10)->select();
        $product_count=$productM->where($whereAC)->select();
        $allcount=count($product_count);
        $allpage=ceil($allcount/10);
        //$this->ajaxReturn(array('product_classify'=>$product_classify));
        $this->ajaxReturn(array('product_classify'=>$product_classify,'product_res'=>$product_res,'allcount'=>$allcount,'allpage'=>$allpage));
    }
}
?>