...
class IndexController extends CommonController {
    ...
    //=============================================================
    //添加底部信息
    public function home_url_add(){
        if($_POST){
            $home_urlM=D('home_url');
            $data['hu_wxerweima']=I('post.hu_wxerweima');
            $data['hu_wburl']=I('post.hu_wburl');
            $data['hu_gifurl']=I('post.hu_gifurl'); 
            $data['hu_telephone']=I('post.hu_telephone'); 
			$data['hu_producturl']=I('post.hu_producturl');
            $whereAA['hu_id']=1;
            $selres=$home_urlM->where($whereAA)->find();
            if($selres){
                $addres=$home_urlM->where($whereAA)->save($data);
            }else{
                $data['hu_id']=1;
                $addres=$home_urlM->add($data);
            }
            
            if($addres){
                $this->success('保存成功',U('Index/home_url'));
            }else{
                $this->error('保存失败',U('Index/home_url'));
            }
            exit;
        }
        $home_urlM=D('home_url');
        $whereAA['hu_id']=1;
        $home_url=$home_urlM->where($whereAA)->find();
        $this->assign('home_url',$home_url);
        $this->display();
    }
    //==============================================================
    //修改底部信息
    public function home_url_update(){
        if($_POST){
            $home_urlM=D('home_url');
            $data['hu_wxerweima']=I('post.hu_wxerweima');
            $data['hu_wburl']=I('post.hu_wburl');
			...
			$data['hu_producturl']=I('post.hu_producturl');
            $whereAA['hu_id']=1;
            $saveres=$home_urlM->where($whereAA)->save($data);
            if($saveres){
                $this->success('保存成功',U('Index/home_url'));
            }else{
                $this->error('保存失败',U('Index/home_url'));
    ...
    //============================================================
    //删除底部信息
    public function home_url_del(){
        $home_urlM=D('home_url');
        $whereAA['hu_id']=1;
        $delres=$home_urlM->where($whereAA)->delete();
        if($delres){
                $this->success('保存成功',U('Index/home_url'));
            }else{
                $this->error('保存失败',U('Index/home_url'));
