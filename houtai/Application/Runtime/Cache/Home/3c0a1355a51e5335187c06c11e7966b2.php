		...
		<div class="Situation">
			<div class="leftbox">
				<div class="sub-nav">
					<div class="nav-title">
						<h3>P</h3>
						<h1><?php echo ($primary_work_classify_res["pwc_name"]); ?></h1>
					...
					<ul>
						<?php if(is_array($primary_work_res)): $k = 0; $__LIST__ = $primary_work_res;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$primary_work): $mod = ($k % 2 );++$k; if($k == $nav_id): ?><li class="on">
									<?php echo ($primary_work["pw_name"]); ?><b>&gt;</b>
								</li>
								<?php else: ?>
								<li>
									<?php echo ($primary_work["pw_name"]); ?><b>&gt;</b>
				...
				<!-- <input type="text" value="<?php echo ($primary_work_classify_res["pw_name"]); ?>" class="PWNAME"> -->
			...
			<div class="Situation-right">
				<?php if(is_array($primary_work_res)): $k = 0; $__LIST__ = $primary_work_res;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$primary_work): $mod = ($k % 2 );++$k; if($k == 1): ?><div>
							<div class="SituationRight-top">
								<span>您当前位置：<a href="/index.php/Home/Index/index">首页</a> > <?php echo ($primary_work_classify_res["pwc_name"]); ?> > <?php echo ($primary_work["pw_name"]); ?></span>
		...
		<script type="text/javascript">
			$(function() {
				...
				$('.sub-nav>ul>li').click(function(){
					var len=$(this).index();
					console.log('lll',len);
					$('.sub-nav>ul>li').eq(len).css('color','red').siblings().css('color','#333');
