...
class IndexController extends CommonController {
    ...
    public function bottomadd(){
        if($_POST){
            ...
            $data['mobile1']=I('post.mobile1');
            $data['qq']=I('post.qq');
            $data['qq1']=I('post.qq1');
            $data['wx_erweima']=I('post.wx_erweima');
            $data['address']=I('post.address');
            $data['record']=I('post.record');
            $data['introduce']=I('post.introduce','',false);
    ...
    public function lunboadd(){
        if($_POST){
            ...
            $data['lb_title_english']=I('post.lb_title_english');
    ...
    public function map(){
		...
		$whereid['id']=1;
		if($_POST){
			$data['heng']=I('post.heng');
			$data['zong']=I('post.zong');
			
			$selres=$mapM->where($whereid)->find();
			if($selres){
				$addres=$mapM->where($whereid)->save($data);
			}else{
				$data['id']=1;
				$addres=$mapM->add($data);
			}
			if($addres){
                $this->success('保存成功',U('Index/map'));
            }else{
                $this->error('保存失败',U('Index/map'));
            }
			exit;
		}
