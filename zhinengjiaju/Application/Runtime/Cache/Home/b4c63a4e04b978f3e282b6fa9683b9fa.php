	...
	<input type="hidden" class="navcss" value="<?php echo ($navfirst["nav_name"]); ?>">
		...
		<div class="Situation">
				...
				<div class="sub-nav">
					...
					<ul>
						<?php if(is_array($navsecond)): $k = 0; $__LIST__ = $navsecond;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$second): $mod = ($k % 2 );++$k; if($k == 1): ?><li class="on">
									...
									<a href="<?php echo ($second["nav_url"]); ?>/nav_id/<?php echo ($second["nav_id"]); ?>/nc_id/<?php echo ($news_classify_res[$k-1]["nc_id"]); ?>"><?php echo ($second["nav_name"]); ?></a><b>&gt;</b>
								</li>
								<?php else: ?>
								<li>
			...
			<div class="Situation-right">
				<?php if(is_array($policy_classify_res)): $k = 0; $__LIST__ = $policy_classify_res;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$policy_classify): $mod = ($k % 2 );++$k; if($k == 1): ?><div>
							...
							<div class="SituationRight-bot">
								<?php if(is_array($policy_article_res[$k-1])): $i = 0; $__LIST__ = $policy_article_res[$k-1];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$policy_article): $mod = ($i % 2 );++$i;?><p><?php echo ($policy_article["pa_title"]); ?>111</p>
									<?php echo ($policy_article["pa_content"]); endforeach; endif; else: echo "" ;endif; ?>
							</div>
				        </div>
				        <?php else: ?>
